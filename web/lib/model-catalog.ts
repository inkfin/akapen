...
export const DEFAULT_PERSONA = "经验丰富、语气专业中立的老师";

export const DEFAULT_PROMPT_SINGLE_SHOT = `你是一位${PERSONA_PLACEHOLDER}，需要在**一次调用内**完成两件事：
...
export const DEFAULT_PROMPT_GRADING = `你是一位${PERSONA_PLACEHOLDER}。基于学生答案的转写文本（OCR 草稿），按下方「本题评分要求」批改：可能是打分（rubric 描述了满分 + 评分点），也可能只给修改建议（rubric 写明"只批注 / 不打分"时）。
...
/**
 * 把 prompt 里的占位符替换成题目上下文：
 * - {persona}：老师级默认人设（留空回落 DEFAULT_PERSONA）
 * - {rubric}：作业背景 + 给分细则 + 修改意见方向 + 可选范文输出要求
 *
 * 兼容策略：
 * - 老模板只有 {rubric} 也没关系；{persona} 缺失时不会报错
 * - 用 split/join 全量替换，避免 String.replace 只替第一处
 * - requireGrading=false 时走 NO_GRADING_BLOCK（只批注）
 */
export function substituteRubric(
  ...
  const resolvedPersona = (persona ?? "").trim() || DEFAULT_PERSONA;
  const subjectText = (batchSubject ?? "").trim();
  const objectiveText = (batchObjective ?? "").trim();
  const modelAnswerGuideText = (modelAnswerGuide ?? "").trim();

  const parts: string[] = [];
  if (subjectText || objectiveText) {
    const lines: string[] = [];
    if (subjectText) lines.push(`- 学科：${subjectText}`);
    if (objectiveText) lines.push(`- 作业目标：${objectiveText}`);
    parts.push(`## 作业背景\n\n${lines.join("\n")}`);
  }
  if (!requireGrading) {
    parts.push(NO_GRADING_BLOCK);
  } else {
    // requireGrading=true 时 rubric 应当非空（前后端双层校验保证）；
    // 但如果意外拿到空串，至少给个兜底提示，让 LLM 知道要按通用作文标准评分，
    // 避免直接把"<空>"原样塞进 prompt。
    parts.push(
      `## 给分细则\n\n${rubricText === "" ? "（老师未填给分细则，按通用作文/答题标准评估并给出合理满分）" : rubricText}`,
    );
  }
  parts.push(
    `## 修改意见方向\n\n${guideText === "" ? DEFAULT_FEEDBACK_GUIDE : guideText}`,
  );
  if (provideModelAnswer) {
    parts.push(
      `## 范文输出要求\n\n请额外输出 \`model_answer\` 字段，提供一份可供学生参考的修改后范文。\n${modelAnswerGuideText === "" ? "" : `\n补充要求：\n${modelAnswerGuideText}`}`.trim(),
    );
  }
  const block = parts.join("\n\n");

  const withRubric = prompt.includes(RUBRIC_PLACEHOLDER)
    ? prompt.split(RUBRIC_PLACEHOLDER).join(block)
    : `${prompt}\n\n# 本题批改要求\n\n${block}\n`;
  if (!withRubric.includes(PERSONA_PLACEHOLDER)) {
    return withRubric;
  }
  return withRubric.split(PERSONA_PLACEHOLDER).join(resolvedPersona);
}
